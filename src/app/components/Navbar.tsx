...
//             const isMainCategory = subCategories[key as keyof typeof subCategories];
...
//                 {/* 🔹 Primary Navigation Mega Menu - Full Screen Width - Always Visible */}
//                 {isMainCategory && (
//                   <div className="absolute top-full left-0 right-0 bg-white shadow-xl rounded-md p-6 z-50 border border-gray-200">
//                     <div className="max-w-7xl mx-auto">
//                       <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
//                         {subCategories[key as keyof typeof subCategories].map((sub: { name: string; slug: string }, idx: number) => (
//                           <Link
//                             key={idx}
//                             href={`/product-category/${key}/${sub.slug}`}
//                             className="block px-4 py-3 text-gray-700 hover:text-sky-500 hover:bg-gray-50 rounded-md transition-colors duration-200"
//                           >
//                             <div className="font-medium">{sub.name}</div>
//                             <div className="text-sm text-gray-500 mt-1">Explore {sub.name.toLowerCase()}</div>
//                           </Link>
//                         ))}
//                       </div>
//                     </div>
  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <nav className="bg-white relative">
        <div className="max-w-7xl mx-auto flex space-x-7 py-3 px-5 text-gray-600 font-medium text-sm">
          {categories.map((category, idx) => {
            const key = category.toLowerCase().replace(/\s+/g, "-");
